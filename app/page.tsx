...
export default function Home(){
  ...
  useEffect(()=>{queueMicrotask(()=>{try{const saved=localStorage.getItem(storeKey);if(saved)setApps(JSON.parse(saved));if(sessionStorage.getItem(entryKey)==="yes")setScreen("today")}catch{}})},[]);
...
function SwipeRow({app,open,setOpen,details,remove}:{app:Application;open:boolean;setOpen:(id:number|null)=>void;details:(id:number)=>void;remove:(id:number)=>void}){const start=useRef(0),delta=useRef(0);const down=(e:PointerEvent<HTMLDivElement>)=>{start.current=e.clientX;delta.current=0;e.currentTarget.setPointerCapture(e.pointerId)};const move=(e:PointerEvent<HTMLDivElement>)=>{delta.current=e.clientX-start.current};const up=()=>{if(delta.current<-48)setOpen(app.id);else if(delta.current>25)setOpen(null);else if(Math.abs(delta.current)<8)details(app.id)};return <article className={`${styles.swipe} ${open?styles.swipeOpen:""}`}><button className={styles.delete} onClick={()=>remove(app.id)}>Delete</button><div className={styles.appRow} role="button" tabIndex={0} onKeyDown={e=>{if(e.key==="Enter"||e.key===" ")details(app.id)}} onPointerDown={down} onPointerMove={move} onPointerUp={up}><span className={styles.drag}>≡</span><section><strong>{app.title}</strong><p>{display(app.organisation)}</p><small>⌖ {display(app.location)}</small>{app.status==="Rejected"&&<em>This application is closed.</em>}</section><span className={`${styles.status} ${app.status==="Rejected"?styles.statusRejected:""}`}>{app.status}</span><span aria-hidden="true">›</span></div></article>}

function Details({app,back,menu,advance,reject,update}:{app:Application;back:()=>void;menu:()=>void;advance:()=>void;reject:()=>void;update:(next:Partial<Application>)=>void}){const index=activeStatuses.indexOf(app.status as Exclude<Status,"Rejected">);const editNotes=()=>{const value=window.prompt("Personal notes",app.notes);if(value!==null)update({notes:value.trim()})};const editDocuments=()=>{const value=window.prompt("Required documents (separate with commas)",app.requiredDocuments.join(", "));if(value!==null)update({requiredDocuments:value.split(",").map(item=>item.trim()).filter(Boolean)})};return <div className={`${styles.page} ${styles.detailPage}`}><header className={styles.flowHeader}><button onClick={back}>‹</button><strong>Job details</strong><button onClick={menu} aria-label="Application options">•••</button></header><div className={styles.detailTable}>{[["▣","Job title",app.title],["▥","Organisation",display(app.organisation)],["⌖","Location",display(app.location)],["▦","Deadline",dateLabel(app.deadline)]].map(([icon,label,value])=><div key={label}><span>{icon}</span><strong>{label}</strong><p>{value}</p></div>)}<button className={styles.detailRowButton} onClick={()=>{if(/^https?:\/\//i.test(app.source))window.open(app.source,"_blank","noopener,noreferrer")}} disabled={!/^https?:\/\//i.test(app.source)}><span>↗</span><strong>Source link</strong><p>{app.source||"Not found — review"}</p><i>›</i></button><div className={styles.progressRow}><span>♧</span><strong>Current status</strong><em className={app.status==="Rejected"?styles.needs:styles.found}>{app.status}</em><div className={styles.progress}>{activeStatuses.map((status,step)=><button key={status} className={app.status!=="Rejected"&&step<=index?styles.progressOn:""} onClick={()=>update({status})}><i/><small>{status}</small></button>)}</div></div><button className={styles.detailRowButton} onClick={editDocuments}><span>▤</span><strong>Required documents</strong><p>{app.requiredDocuments.join(", ")||"Not found — review"}</p><i>›</i></button><button className={styles.detailRowButton} onClick={editNotes}><span>⌕</span><strong>Personal notes</strong><p>{app.notes||"Add notes…"}</p><i>›</i></button><button className={styles.detailRowButton} onClick={()=>update({reminder:!app.reminder})}><span>⌁</span><strong>Reminder settings</strong><p>{app.reminder?"Active":"Off"}</p><i>{app.reminder?"On":"Off"}</i></button></div>{app.status==="Rejected"?<div className={styles.closed}>This application is closed.</div>:<div className={styles.detailActions}><button onClick={advance}>Update status</button><button onClick={reject}>Mark as rejected</button></div>}</div>}
