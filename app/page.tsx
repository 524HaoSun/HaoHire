...
function Today({source,setSource,importJob,error,upcoming,openDetails,addFocus,settings}:{source:string;setSource:(v:string)=>void;importJob:(e:FormEvent)=>void;error:string;upcoming:Application[];openDetails:(id:number)=>void;addFocus:()=>void;settings:()=>void}){
  return <div className={styles.page}><header className={styles.todayHeader}><div><p>GOOD MORNING</p><h1>Ready for your<br/>next move?</h1></div><button type="button" onClick={settings} aria-label="Settings">⚙</button></header>
    ...
    <section className={styles.deadlines}><header className={styles.deadlineHeader}><div><p>UP NEXT</p><h2>Deadlines</h2></div><span>{upcoming.length} active</span></header>{upcoming.length?<div className={styles.deadlineList}>{upcoming.map(app=><button className={deadlineTone(app.deadline)} key={app.id} onClick={()=>openDetails(app.id)}><OrganisationLogo app={app}/><div><strong>{app.title}</strong><small>{display(app.organisation)}</small></div><time><strong>{deadlineCountdown(app.deadline)}</strong><small>{dateLabel(app.deadline)}</small></time></button>)}</div>:<Empty title="You are all caught up" copy="No active deadlines right now. Add a job whenever you are ready." action="Add another job" onClick={addFocus} icon="✓"/>}</section>
  </div>
}
