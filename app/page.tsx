...
type EditableKey = "title"|"organisation"|"location"|"deadline"|"employmentType"|"source"|"notes";
...
export default function Home(){
  ...
  const quickAdvance=(id:number)=>{const target=apps.find(app=>app.id===id);if(!target||target.status==="Rejected"||target.status==="Offer")return;const index=activeStatuses.indexOf(target.status as Exclude<Status,"Rejected">);const next=activeStatuses[index+1];setApps(current=>current.map(app=>app.id===id?{...app,status:next}:app));setStatusUndo({id,previous:target.status,next});setOpenRow(null);window.setTimeout(()=>setStatusUndo(null),5000)};
  const restoreStatus=()=>{if(!statusUndo)return;setApps(current=>current.map(app=>app.id===statusUndo.id?{...app,status:statusUndo.previous}:app));setStatusUndo(null)};
  ...
  return <main className={styles.stage}><section className={styles.phone}>
    ...
    {screen==="applications"&&<Applications apps={apps} counts={counts} filter={filter} setFilter={setFilter} showFilter={()=>setOverlay("filter")} openRow={openRow} setOpenRow={setOpenRow} openDetails={openDetails} deleteApp={deleteApp} advanceStatus={quickAdvance} add={()=>go("today")}/>}
    {screen==="details"&&selected&&<Details app={selected} back={()=>go("applications")} menu={()=>setOverlay("detailsMenu")} reject={()=>setRejectOpen(true)} update={next=>setApps(current=>current.map(app=>app.id===selected.id?{...app,...next}:app))}/>}
...
function Today({source,setSource,importJob,error,upcoming,openDetails,addFocus,settings}:{source:string;setSource:(v:string)=>void;importJob:(e:FormEvent)=>void;error:string;upcoming:Application[];openDetails:(id:number)=>void;addFocus:()=>void;settings:()=>void}){
  const featured=upcoming.find(app=>daysUntil(app.deadline)!==null);
  const remaining=featured?upcoming.filter(app=>app.id!==featured.id):upcoming;
...
function Details({app,back,menu,reject,update}:{app:Application;back:()=>void;menu:()=>void;reject:()=>void;update:(next:Partial<Application>)=>void}){
  ...
  const saveEditor=()=>{if(!editor)return;update({[editor.key]:editValue.trim()} as Partial<Application>);setEditor(null)};
